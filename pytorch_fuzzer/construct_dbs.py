...
def how_many_needed(target_type):
    if target_type[0].islower():
        # return 1000 # primitive, should be fine to have many of them
        return 1000
                    ...
                    for line in infile:
                        ...
                        if not line.startswith('==='):
                            ...
                            if way not in has_seen:
                                ...
                                way = '# ' + f + '\n' + 'import torch\n' + 'import pytorch_fuzzingbook_invariant_utils\n' + way + ')'
...
def write_and_run_way(way_and_identifier_and_funcnamemap):
    ...
    with open('/tmp/tmp__test_' + identifier + '.py', 'w+') as outfile:
        ...
        # outfile.write('pytorch_fuzzingbook_invariant_utils.get_invariants_hold(pytorch_fuzzingbook_invariant_utils.INVARIANT_PROPERTIES, ' + way + ' )')
        outfile.write(way)
    ...
    with open('/tmp/tmp__test_' + identifier + '.py', 'w+') as outfile:
        outfile.write('from torch import *\n')
        outfile.write(way)
    ...
    for way, func_name_to_num_call_to_arg_name in ways:
        for line in way.split('\n'):
            if line.startswith('import '):
                ...
                imports[imported_thing].add('import torch\n')
                imports[imported_thing].add('from numpy import inf')
                imports[imported_thing].add('from numpy import nan')
...
print('copied pytorch_fuzzingbook_invariant_utils.py to tmp folder')
shutil.copyfile('pytorch_fuzzingbook_invariant_utils.py', '/tmp/pytorch_fuzzingbook_invariant_utils.py')
