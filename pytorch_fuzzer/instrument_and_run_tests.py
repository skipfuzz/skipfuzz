...
class TorchProxy(object):
    ...
    def func_decorator(self, func):
        def wrapper(*args, **kwargs):
            ...
            code_str = 'torch.' + func.__name__ + '(' + ','.join(args_repr) + ',' + ','.join(kwargs_repr)  + ')'
           ...
           # print('id=', id(result))
            ...
            if type(self.torch).__name__ == 'module':
                name = 'torch.' + func.__name__ + '('
            else:
                # some other torch object
                name = type(self.torch).__name__ + '.' + func.__name__ + '('
    ...
    def __getattr__(self, name):
        try:
            ret = getattr(self.torch, name)
            if type(ret).__name__ == 'builtin_function_or_method':
                return self.func_decorator(ret)
            if type(ret).__name__ == 'type':
                result = ret
                # print('torch type access', result)
                object_graph[id(result)] = (name, 'torch.' + name, dict())
                function_call_count['torch.' + name] += 1
                typed_to_objects[type(result).__name__].add(id(result))
...
def string_match_function_receiver(line, function_name):
    ...
    if function_name.endswith( '__init__'): # ctor
...
def is_right_frame(frame):
    ...
    try:
        ...
        expected_call = frame.f_code.co_name 
...
def create_mod_test(original_filename):
    ...
    for line in prettified_code_.split('\n'):
        if (line.startswith('import')  or line.startswith('from')) and 'import' in line and 'numpy' in line and 'as np' in line:
            imports.append(line.strip())

            indentation = ' ' * (len(line) -len(line.lstrip()))
            prettified_code += indentation + 'pass' + '\n'
            continue
        elif (line == 'import torch'):
            imports.append(line.strip())
    ...
    with open(modified_file, 'w+') as outfile:
        ...
        outfile.write(prettified_code.replace("if (__name__ == '__main__'):", "sys.settrace(traceit)\nif (__name__ == '__main__'):"))
        outfile.write("    " )
...
def collect_initialization_graphs():
    ...
    for root, subdirs, files in os.walk(rootdir):
        ...
        with Pool(max(cpu_count() // 2 + 1, 1)) as pool:
            modified_filenames = []
            for f in files:
                if not f.endswith('.py') or 'test' not in f or f.startswith('mod_'):
                    continue
...
def run(modified_filename):
    print('will run', modified_filename)
    proc = subprocess.Popen(['timeout','--signal=SIGKILL', '10000',  "python", "-m", 'pytest', modified_filename], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
